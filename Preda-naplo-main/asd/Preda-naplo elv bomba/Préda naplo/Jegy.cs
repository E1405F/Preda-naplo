...
namespace Préda_naplo
{
    internal class Jegy
    {
        ...
        public double Suly { get; set; } // 1.0 = 100%, 1.5 = 150%, stb.
        ...
        public override string ToString()
        {
            ...
            string fellebbezesInfo = Fellebbezes ?
                (FellebbezesElbiralva ? " [✅ Fellebbezés elbírálva]" : " [⚠️ Fellebbezés benyújtva]") : "";

            return $"{Tantargy} - {Ertek}{sulyInfo} ({Datum:yyyy.MM.dd}) - {TanarFelhasznalonev}" +
                   (string.IsNullOrEmpty(Megjegyzes) ? "" : $" - {Megjegyzes}") + fellebbezesInfo;
        }

        public string ToFileFormat()
        {
            return $"{DiakFelhasznalonev};{TanarFelhasznalonev};{Tantargy};{Ertek};{Datum:yyyy.MM.dd};{Megjegyzes};{Suly};{Fellebbezes};{FellebbezesIndoklas};{FellebbezesElbiralva}";
        }
