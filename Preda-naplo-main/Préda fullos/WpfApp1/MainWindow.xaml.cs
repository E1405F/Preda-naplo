...
namespace PredaNaplo
{
    public partial class MainWindow : Window
    {
            ...
            foreach (var obj in MenuListBox.Items)
            {
                ...
                switch (item.Tag?.ToString())
                {
                    ...
                    case "Statistics":
                        ...
                        break;
                    case "Appeal":                     // ÚJ
                        item.Visibility = (szerepkor == "Diák") ? Visibility.Visible : Visibility.Collapsed;
                        break;
                }
            }
        }
            ...
            if (MenuListBox?.SelectedItem is ListBoxItem selected && selected.Tag != null)
            {
                switch (selected.Tag.ToString())
                {
                    ...
                    case "Absences":
                        ...
                        break;
                    case "Appeal":                     // ÚJ
                        ShowAppealSubmission();
                        break;
                }
            }
        }
        private void ShowAppealSubmission()
        {
            ContentPanel?.Children.Clear();

            var title = CreateTitle("Fellebbezés benyújtása");
            ContentPanel.Children.Add(title);

            var studentGrades = jegyManager?.GetJegyekByDiak(aktualisFelhasznalo.Felhasznalonev) ?? new List<Jegy>();
            var appealableGrades = studentGrades.Where(j => !j.Fellebbezes).ToList();

            if (!appealableGrades.Any())
            {
                var card = CreateCard();
                card.Child = new TextBlock
                {
                    Text = "Nincsenek fellebbezhető jegyeid (minden jegyhez már benyújtottál fellebbezést vagy a jegy már elbírálás alatt van).",
                    FontSize = 14,
                    Foreground = GetBrush("TextColor", Brushes.Gray),
            ...
            grid.Columns.Add(new DataGridTextColumn { Header = "Tantárgy", Binding = new Binding("Tantargy"), Width = 100 });
            grid.Columns.Add(new DataGridTextColumn { Header = "Jegy", Binding = new Binding("Ertek"), Width = 60 });
            grid.Columns.Add(new DataGridTextColumn { Header = "Dátum", Binding = new Binding("DatumString"), Width = 100 });
            grid.Columns.Add(new DataGridTextColumn { Header = "Tanár", Binding = new Binding("TanarNev"), Width = 120 });
            grid.Columns.Add(new DataGridTextColumn { Header = "Megjegyzés", Binding = new Binding("Megjegyzes"), Width = 200 });

            var items = appealableGrades.Select(j => new
            {
                j.Tantargy,
                j.Ertek,
                DatumString = j.Datum.ToString("yyyy.MM.dd"),
                TanarNev = felhasznalok.FirstOrDefault(f => f.Felhasznalonev == j.TanarFelhasznalonev)?.Nev ?? j.TanarFelhasznalonev,
                j.Megjegyzes,
                Jegy = j
            }).ToList();
            ...
            cardGrid.Child = grid;
            ContentPanel.Children.Add(cardGrid);

            var appealButton = new Button
            {
                Content = "Fellebbezés benyújtása",
                Style = GetStyle("ModernButton"),
                Width = 200,
                Margin = new Thickness(0, 10, 0, 0),
                HorizontalAlignment = HorizontalAlignment.Center
            };
            appealButton.Click += (s, e) =>
            {
                if (grid.SelectedItem == null)
                {
                    MessageBox.Show("Válassz ki egy jegyet!", "Figyelmeztetés", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                dynamic selected = grid.SelectedItem;
                Jegy selectedJegy = selected.Jegy;

                var dialog = new Window
                {
                    Title = "Fellebbezés indoklása",
                    Width = 450,
                    Height = 350,                 // megnövelt magasság
                    SizeToContent = SizeToContent.Height, // automatikus magasság a tartalomhoz
                    WindowStartupLocation = WindowStartupLocation.CenterOwner,
                    Owner = this
                };
                var stack = new StackPanel { Margin = new Thickness(20) };
                stack.Children.Add(new TextBlock { Text = "Indoklás:", Margin = new Thickness(0, 10, 0, 5) });
                var reasonBox = new TextBox { Style = GetStyle("ModernTextBox"), Height = 100, TextWrapping = TextWrapping.Wrap };
                ...
                var submitBtn = new Button { Content = "Benyújtás", Style = GetStyle("ModernButton"), Width = 100 };
                ...
                submitBtn.Click += (s2, e2) =>
                {
                    ...
                    {
                        MessageBox.Show("Az indoklás nem lehet üres!", "Hiba", MessageBoxButton.OK, MessageBoxImage.Error);
                        return;
                    }
                    jegyManager.SubmitAppeal(selectedJegy, reasonBox.Text);
                    dialog.Close();
                    ShowAppealSubmission(); // frissítés
                };
            ...
            var appealCard = CreateCard();
            appealCard.Child = appealButton;
            ContentPanel.Children.Add(appealCard);
        }
        ...
        private void ShowUserManagement()
        {
            ...
            var toggleBtn = new Button { Content = "Letiltás/Feloldás", Style = GetStyle("ModernButton"), Width = 160, Margin = new Thickness(5) };
            var changePasswordBtn = new Button { Content = "Jelszó változtatás", Style = GetStyle("ModernButton"), Width = 160, Margin = new Thickness(5) };
        ...
        private void ShowRecordAbsenceDialog()
        {
            ...
            // Diákok listájának összeállítása
            if (aktualisFelhasznalo is Tanar tanar)
            {
                if (tanar.TanitottOsztalyok.Any())
                {
                    var diakok = felhasznalok
                    ...
                    foreach (var d in diakok) diakCombo.Items.Add(d);
                }
                else
                {
                    MessageBox.Show("Nincs jogosultságod hiányzást rögzíteni, mert nincsenek tanított osztályaid!",
                                    "Figyelmeztetés", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
            }
            else // igazgató vagy admin
            {
                var diakok = felhasznalok
            ...
            var tantargyCombo = new ComboBox { Margin = new Thickness(0, 0, 0, 10), FontSize = 14 };
            if (aktualisFelhasznalo is Tanar tanarObj)
            {
        ...
        private void ShowJustifyAbsenceDialog()
        {
            ...
            if (aktualisFelhasznalo?.Szerepkor == "Tanár" || aktualisFelhasznalo?.Szerepkor == "Igazgató")
            {
                ...
                // Diákok listájának összeállítása tanár/igazgató esetén
                if (aktualisFelhasznalo is Tanar tanar)
                {
                    if (tanar.TanitottOsztalyok.Any())
                    {
                    ...
                    {
                        MessageBox.Show("Nincs jogosultságod hiányzást igazolni, mert nincsenek tanított osztályaid!",
                                        "Figyelmeztetés", MessageBoxButton.OK, MessageBoxImage.Warning);
                        return;
                    }
                }
            ...
            {
                // Szülői rész (változatlan, a korábban megadott kód)
                var gyerek = GetGyermek(aktualisFelhasznalo);
                if (gyerek == null)
                {
                    stack.Children.Add(new TextBlock
