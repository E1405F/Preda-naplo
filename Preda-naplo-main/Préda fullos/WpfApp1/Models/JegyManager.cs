...
namespace WpfApp1.Models
{
    internal class JegyManager
    {
        ...
        public void SubmitAppeal(Jegy jegy, string indoklas)
        {
            jegy.Fellebbezes = true;
            jegy.FellebbezesIndoklas = indoklas;
            jegy.FellebbezesElbiralva = false;
            FrissitFajl();
        }
        ...
        // Existing console-oriented methods kept for compatibility (not removed)...
        ...
        public void JegyekListazasa(Felhasznalo felhasznalo)
        {
            ...
            {
                // Szülő esetén a gyermek jegyeit kell megjeleníteni
                megjelenitendoJegyek = GetGyermekJegyei(felhasznalo);
            }
            else
            {
                // Diák esetén a saját jegyeit
        ...
        // ÚJ METÓDUS: Gyermek jegyeinek lekérése szülő számára
        private List<Jegy> GetGyermekJegyei(Felhasznalo szulo)
        {
            ...
            else
            {
                ...
                Console.WriteLine("Kérjük, vedd fel a kapcsolatot az adminisztrátorral!");
            }

            return gyermekJegyei;
        }

        // ÚJ METÓDUS: Gyermek felhasználónév lekérése
        private string GetGyermekFelhasznalonev(Felhasznalo szulo)
        {
            // Itt kellene hozzáférni a szülő-gyermek kapcsolathoz
            // Mivel nincs direkt hozzáférésünk, használjuk a fájlból való betöltést
            string szuloGyerekFajl = "szulogyerek.txt";

            if (File.Exists(szuloGyerekFajl))
            {
                foreach (var sor in File.ReadAllLines(szuloGyerekFajl))
                {
                    if (string.IsNullOrWhiteSpace(sor)) continue;
                    var adatok = sor.Split(';');
                    if (adatok.Length >= 2 && adatok[0] == szulo.Felhasznalonev)
                    {
                        return adatok[1];
                    }
                }
        ...
        // New methods used by the WPF UI:

        public List<Jegy> GetOsszesJegy()
        {
            return jegyek.ToList();
        }

        public List<Jegy> GetJegyekByDiak(string diakFelhasznalonev)
        {
            return jegyek.Where(j => j.DiakFelhasznalonev == diakFelhasznalonev).ToList();
        }

        public List<Jegy> GetFellebbezesek()
        {
            return jegyek.Where(j => j.Fellebbezes && !j.FellebbezesElbiralva).ToList();
        }

        public void FellebbezesElfogadas(Jegy jegy, int ujErtek)
        {
            jegy.Ertek = ujErtek;
            jegy.FellebbezesElbiralva = true;
            // ensure fellebbezes flag reset if appropriate
            jegy.Fellebbezes = false;
            FrissitFajl();
        }

        public void FellebbezesElutasitas(Jegy jegy)
        {
            jegy.FellebbezesElbiralva = true;
            // keep Fellebbezes = true to indicate it was appealed but processed, or set to false depending on domain logic
            FrissitFajl();
        }

        public void UjJegy(Felhasznalo tanar, string diakFelhasznalonev, string tantargy, int ertek, string megjegyzes = "")
        {
            if (tanar.Szerepkor != "Tanár")
                throw new InvalidOperationException("Csak tanárok rögzíthetnek jegyeket.");

            var diak = felhasznalok.FirstOrDefault(f => f.Felhasznalonev == diakFelhasznalonev && f.Szerepkor == "Diák");
            if (diak == null)
                throw new InvalidOperationException("A megadott diák nem található.");

            var uj = new Jegy(diakFelhasznalonev, tanar.Felhasznalonev, tantargy, ertek, DateTime.Now, megjegyzes);
            jegyek.Add(uj);
            File.AppendAllText(fajlEleres, uj.ToFileFormat() + Environment.NewLine);
        }

        public void UjJegySullyal(Felhasznalo tanar, string diakFelhasznalonev, string tantargy, int ertek, double suly, string megjegyzes = "")
        {
            if (tanar.Szerepkor != "Tanár")
                throw new InvalidOperationException("Csak tanárok rögzíthetnek jegyeket.");
            ...
            var uj = new Jegy(diakFelhasznalonev, tanar.Felhasznalonev, tantargy, ertek, DateTime.Now, megjegyzes, suly);
            jegyek.Add(uj);
            File.AppendAllText(fajlEleres, uj.ToFileFormat() + Environment.NewLine);
        }
