...
namespace PredaNaplo
{
    public partial class MainWindow : Window
    {
        ...
        private Felhasznalo selectedUserForManagement;
        ...
        private void LoginButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (user.EllenorizJelszo(password))
            {
                ...
                // DEBUG: Tanár esetén írd ki a tantárgyakat
                if (aktualisFelhasznalo is Tanar tanar)
                {
                    System.Diagnostics.Debug.WriteLine($"Bejelentkezett tanár: {tanar.Nev}");
                    System.Diagnostics.Debug.WriteLine($"Tantárgyak száma: {tanar.Tantargyak.Count}");
                    foreach (var t in tanar.Tantargyak)
                    {
                        System.Diagnostics.Debug.WriteLine($"  - {t}");
                    }

                    // Ha nincs tantárgy, írj ki figyelmeztetést
                    if (tanar.Tantargyak.Count == 0)
                    {
                        MessageBox.Show("Figyelmeztetés: A tanárnak nincsenek tantárgyai beállítva! Ellenőrizd a felhasznalok.txt fájlt.", "Figyelmeztetés", MessageBoxButton.OK, MessageBoxImage.Warning);
                    }
        ...
        private void LoadDefaultContent()
        {
            ...
            // Tanár esetén tantárgyak kiírása
        ...
        private void ShowNewGradeDialog()
        {
            ...
            // Diák választása legördülő listából
            ...
            // Tantárgy választása (csak a tanár saját tantárgyai)
            stack.Children.Add(new TextBlock { Text = "Tantárgy:", Margin = new Thickness(0, 10, 0, 5) });
            var tantargyCombo = new ComboBox
            {
                Margin = new Thickness(0, 0, 0, 10),
                FontSize = 14
            };
            ...
            if (aktualisFelhasznalo is Tanar tanar)
            {
                foreach (var t in tanar.Tantargyak)
                {
                    tantargyCombo.Items.Add(t);
                }
            }

            if (tantargyCombo.Items.Count > 0)
                tantargyCombo.SelectedIndex = 0;
            ...
            saveBtn.Click += (s, e) =>
            {
                ...
                {
                    message.Text = "❌ Válassz ki egy diákot!";
                    return;
                }

                if (tantargyCombo.SelectedItem == null || tantargyCombo.Items.Count == 0)
                {
                    message.Text = "❌ Nincs tantárgy kiválasztva! A tanárnak nincsenek tantárgyai beállítva.";
                    return;
                }
        ...
        private void ShowUserManagement()
        {
            ...
            toggleBtn.Click += (s, e) =>
            {
                ...
                {
                    MessageBox.Show("Válassz ki egy felhasználót!", "Figyelmeztetés", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                if (user.Felhasznalonev == aktualisFelhasznalo?.Felhasznalonev)
                {
                    MessageBox.Show("Nem tilthatod le saját magad!", "Hiba", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                // Frissítsd a DataGrid tartalmát anélkül, hogy újraépítenéd az oldalt
                var updatedList = felhasznalok.Select(f => new
                {
                    f.Nev,
                    f.Felhasznalonev,
                    f.Szerepkor,
            ...
            deleteBtn.Click += (s, e) =>
            {
                ...
                if (user.Felhasznalonev == aktualisFelhasznalo?.Felhasznalonev)
                {
                    MessageBox.Show("Nem törölheted saját magad!", "Hiba", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                if (MessageBox.Show($"Biztosan törlöd a(z) {user.Nev} felhasználót?", "Megerősítés", MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes)
                {
                    ...
                    // Frissítsd a DataGrid tartalmát
                    var updatedList = felhasznalok.Select(f => new
                    {
                        f.Nev,
                        f.Felhasznalonev,
            ...
            changePasswordBtn.Click += (s, e) =>
            {
                ...
                if (dlg.ShowDialog() == true && dlg.IsConfirmed)
                {
                    ...
                    // Csak a DataGrid tartalmát frissítsük, ne az egész oldalt
                    var updatedList = felhasznalok.Select(f => new
                    {
                        f.Nev,
                        f.Felhasznalonev,
        ...
        private void ShowRegistrationDialog()
        {
            ...
            // Iskola neve - ComboBox (csak a megadott iskola)
            // Iskola neve - ComboBox (csak a megadott iskola)
            ...
            roleCombo.SelectionChanged += (s, e) =>
            {
                ...
                else if (role == "Szülő")
                {
                    ...
                    // Gyermekválasztó létrehozása, ha még nincs
                    if (childCombo == null)
                    {
            ...
            saveBtn.Click += (s, e) =>
            {
                ...
                {
                    message.Text = "❌ Felhasználónév megadása kötelező!";
                    return;
                }
                if (string.IsNullOrWhiteSpace(passwordBox.Password))
                {
                    message.Text = "❌ Jelszó megadása kötelező!";
                    return;
                }
                if (string.IsNullOrWhiteSpace(nameBox.Text))
                {
                    message.Text = "❌ Teljes név megadása kötelező!";
                    return;
                }
                if (schoolCombo.SelectedItem == null)
                {
                    message.Text = "❌ Iskola kiválasztása kötelező!";
                    return;
                }
                if (roleCombo.SelectedItem == null)
                {
                    message.Text = "❌ Szerepkör kiválasztása kötelező!";
                    return;
                }

                string role = roleCombo.SelectedItem.ToString();

                // Osztály ellenőrzése (csak ha látható és elérhető)
                if (classCombo.Visibility == Visibility.Visible && classCombo.IsEnabled)
                {
                    if (classCombo.SelectedItem == null || string.IsNullOrWhiteSpace(classCombo.SelectedItem.ToString()))
                    {
                        message.Text = "❌ Osztály kiválasztása kötelező!";
                        return;
                    }
                }

                // Tanár esetén tantárgyak ellenőrzése
                List<string> kivalasztottTantargyak = new List<string>();
                if (role == "Tanár")
                {
                    foreach (var item in tantargyListBox.SelectedItems)
                        kivalasztottTantargyak.Add(item.ToString());
                    if (kivalasztottTantargyak.Count == 0)
                    {
                        message.Text = "❌ Tanár esetén legalább egy tantárgyat ki kell választani!";
                        return;
                    }
                }

                // Szülő esetén gyermek ellenőrzése
                string gyerekFelh = null;
                if (role == "Szülő")
                {
                    if (childCombo == null || childCombo.SelectedItem == null)
                    {
                        message.Text = "❌ Szülő esetén gyermek kiválasztása kötelező!";
                        return;
                    }
                ...
                // Titkos kód ellenőrzése (csak bizonyos szerepköröknél)
                if (role == "Igazgató" || role == "Adminisztrátor" || role == "Tanár")
                {
                    if (string.IsNullOrWhiteSpace(codeBox.Password))
                    {
                        message.Text = "❌ Titkos kód megadása kötelező!";
                        return;
                    }
                    if (codeBox.Password != TITKOS_KOD)
                    {
                ...
                string osztaly = (classCombo.Visibility == Visibility.Visible && classCombo.IsEnabled && classCombo.SelectedItem != null)
                    ? classCombo.SelectedItem.ToString() : "";
                ...
                if (success)
                {
                    ...
                    dialog.Close();
                    // Ha adminisztrátor vagy igazgató, frissítsd a felhasználókezelő nézetet
        ...
        private void ShowRecordAbsenceDialog()
        {
            ...
            saveBtn.Click += (s, e) =>
            {
                ...
                // Kiválasztott diák felhasználónevének lekérése
                dynamic selectedDiak = diakCombo.SelectedItem;
                string diakFelhasznalonev = selectedDiak.Felhasznalonev;
