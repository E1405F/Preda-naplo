...
namespace WpfApp1.Models
{
    internal class HianyzasManager
    {
        ...
        // ========== ÚJ METÓDUSOK A WPF-HOZ ==========

        public List<Hianyzas> GetOsszesHianyzas()
        {
            return hianyzasok.ToList();
        }

        public List<Hianyzas> GetHianyzasokByDiak(string diakFelhasznalonev)
        {
            return hianyzasok.Where(h => h.DiakFelhasznalonev == diakFelhasznalonev).ToList();
        }

        public List<Hianyzas> GetIgazolatlanHianyzasok()
        {
            return hianyzasok.Where(h => !h.Igazolt).ToList();
        }

        public List<Hianyzas> GetIgazolatlanHianyzasokByDiak(string diakFelhasznalonev)
        {
            return hianyzasok.Where(h => h.DiakFelhasznalonev == diakFelhasznalonev && !h.Igazolt).ToList();
        }

        public void HianyzasRogzitese(Felhasznalo tanar, string diakFelh, DateTime datum, string ora)
        {
            if (tanar.Szerepkor != "Tanár")
            {
                System.Windows.MessageBox.Show("❌ Csak tanárok rögzíthetnek hiányzásokat!", "Hiba",
            ...
            if (diak == null)
            {
                System.Windows.MessageBox.Show("❌ Nem található ilyen diák!", "Hiba",
                    System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Error);
                return;
            }
        ...
        public void Igazolas(Hianyzas hianyzas)
        {
            hianyzas.Igazolt = true;
            FrissitFajl();
        }
